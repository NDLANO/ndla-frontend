  ...
  return (
    <Button onClick={onClick} {...rest}>
      <SaveButtonLabel aria-hidden>
        <SaveText saved={saved}>{t("myNdla.resource.save")}</SaveText>
        <AddedText saved={saved}>{t("myNdla.resource.added")}</AddedText>
      </SaveButtonLabel>
      <HeartContainer aria-hidden>
        <StyledSpinner visible={!triggered && !!loading} />
        <StyledHeartLine invisible={saved || (!triggered && loading)} />
        <StyledHeartFill visible={saved} />
        <MovingHeart1 triggered={!!loading && triggered} />
        <MovingHeart2 triggered={!!loading && triggered} />
        <MovingHeart3 triggered={!!loading && triggered} onAnimationEnd={() => setTriggered(false)} />
      </HeartContainer>
    </Button>
  );
};
