  ...
  MML.menclose.Augment({
    toCommonHTML: function(node) {
      ...
      var notations = MathJax.Hub.SplitList(values.notation),
        notation = {};
      for (var i = 0, m = notations.length; i < m; i++)
        notation[notations[i]] = true;
    ...
    CHTMLnotation: {
      ...
      updiagonalstrike: function(child, cbox, bb, p, t, SOLID) {
        var H = bb.H,
          D = bb.D,
          W = bb.W;
        var svg = this.CHTMLsvg(child, bb, t, cbox.scale);
    ...
    //  contents
    //
    CHTMLsvg: function(node, bbox, t, scale) {
      if (!svg) {
        ...
        if (svg.style && scale) {
          ...
          svg.style.width = CHTML.Em(bbox.W * s);
          svg.style.height = CHTML.Em(bbox.H + bbox.D * s);
          svg.style.verticalAlign = CHTML.Em(-bbox.D * s);
          svg.style.marginLeft = CHTML.Em(-bbox.W * s);
        } else if (svg.style) {
