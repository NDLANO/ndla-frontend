...
export const isLearningPathResource = (contentUri: string | null | undefined): contentUri is string =>
  !!contentUri?.startsWith(URN_LEARTNING_PATH);

export const isArticleResource = (contentUri: string | null | undefined): contentUri is string =>
  !!contentUri?.startsWith(URN_ARTICLE) && contentUri.length > 12;

export const getArticleIdFromResource = (contentUri: string | null | undefined) => {
  return isArticleResource(contentUri) ? contentUri.replace(URN_ARTICLE, "") : undefined;
};

export const getLearningPathIdFromResource = (contentUri: string | null | undefined) => {
