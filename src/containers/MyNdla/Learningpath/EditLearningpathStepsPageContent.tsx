...
import { LearningpathOutlet } from "./LearningpathOutlet";
...
export const EditLearningpathStepsPageContent = ({ learningpath }: Props) => {
  ...
  useEffect(() => {
    const castedLocationState = location.state as LocationState;
    const focusId = castedLocationState?.focusStepId ?? stepId;
    if (!focusId) return;
