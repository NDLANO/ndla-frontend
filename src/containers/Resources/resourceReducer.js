...
import { getArticleIdFromResource, getLearningPathIdFromResource } from './resourceHelpers';
  ...
  [actions.setLearningPathResourceData]: {
    next: (state, action) => {
      ...
      const resources = state.all[topicId].map((resource) => {
        ...
        if (learningPathId) {
          const learningPath = learningPathResourceData.find(lp => lp.id.toString() === learningPathId);
          if (learningPath) {
