...
test('can navigate to folders', async ({ page }) => {
  ...
  await page
    .getByRole('listitem')
    .getByRole('link', { name: 'Mine mapper' })
    .click();
  await expect(
...
test('can navigate to subjects', async ({ page }) => {
  ...
  await page
    .getByRole('listitem')
    .getByRole('link', { name: 'Mine favorittfag' })
    .click();
  await expect(
    page.getByRole('heading').getByText('Mine favorittfag'),
  ).toBeVisible();
});
...
test('can navigate to tags', async ({ page }) => {
  ...
  await page
    .getByRole('listitem')
    .getByRole('link', { name: 'Mine emneknagger' })
    .click();
  await expect(
    page.getByRole('heading').getByText('Mine emneknagger'),
  ).toBeVisible();
});
...
test('can navigate to profile', async ({ page }) => {
  ...
  await page
    .getByRole('listitem')
    .getByRole('link', { name: 'Min profil' })
    .click();
